...
module ShopifyCLI
  module Theme
    class SyncerTest < Minitest::Test
      ...
      def test_download_theme_with_query_filter
        @syncer.query_filter = mock("QueryFilter")
        @syncer.query_filter.stubs(:match?).returns(false)
        @syncer.query_filter.expects(:match?).with("layout/theme.liquid").returns(true)
